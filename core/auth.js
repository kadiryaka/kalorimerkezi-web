...
module.exports = {
    ...
    "secure": function (req, res, next) {
        ...
        connection.query('SELECT * from tokens t inner join kullanici k ON t.user_id = k.k_id where t.token = ? ', [token], function (err, result) {
            console.log("yetki asda e girdi")
            if (result.length === 0) {
                console.log("yetki fdgd e girdi")
                ...
                console.log("123HTTP isteği alındı, istekle eşleşen token var yetki ve kullanici adi atandi");
                req.user_id = result[0].user_id;
                req.auth = result[0].yetki;
                next();
            }
